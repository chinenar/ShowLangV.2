...
When the layout actually changes, ShowLang waits 45 ms for the Windows language switch to settle, tries the inexpensive Win32 caret, then queries a preloaded `ShowLang.exe --caret-worker` process. If the provider answers quickly with no caret, ShowLang may retry after a short delay up to two times to survive focus handoff races. Slow/time-out responses are never retried. The worker is blocked on a pipe between requests and performs no caret inspection while idle.
